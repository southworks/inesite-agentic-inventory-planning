...
- `.statistical_anomaly_weeks` — weeks containing either a global daily outlier
  (`|units - mean| > 2.5 * stdev`) or an unexplained run of three very-low consecutive
  daily sales values; promo and holiday weeks are not double-counted as unexplained low-run
  anomalies
...
- `statistical_anomaly_weeks` is a genuine computed feature, not a lookup of the known
  `ANOMALY-01`/`ANOMALY-02` scenarios — `scenario_refs` is the lookup. The two should usually
  agree; if they don't, that itself is interesting (a real anomaly the constants don't capture,
  or a flagged week with no actual outlier).
