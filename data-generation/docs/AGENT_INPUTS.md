...
csv/txt exports and **co-located by source type** under `data-generation/source-exports/ or expected-outputs/_full_exports/<source_type>/`.
...
(`expected-outputs/IPF-XXX_<path>/01_signal_ingestion/input/<source_type>/`) so each test case is a
...
Paths are relative to `source-exports/_full_exports/<source_type>/` (the canonical store).
...
**60 PDFs + 6 PNGs = 66 files** under `data-generation/source-exports/ or expected-outputs/_full_exports/<source_type>/`. The shipment,
...
Re-run after editing the canonical `source-exports/_full_exports/<source_type>/*.csv|*.txt` (or
...
source-exports/_full_exports/ → generate_normalized_layers.py → 01_pos_transactions/ ... 07_decision_ground_truth/
                      → build_scenario_folders.py     → expected-outputs/IPF-XXX_<path>/ (two MCP stages)
```

`generate_normalized_layers.py` reads only the canonical csv/txt under `data-generation/source-exports/ or expected-outputs/_full_exports/`;
...
1. Add the new raw file under `data-generation/source-exports/ or expected-outputs/_full_exports/<source_type>/` (and re-run `generate_raw_layer.py` if it's derived from the M5 extract).
