...
## What lives under `data-generation/source-exports/ or expected-outputs/` (source of truth for demos)

Each scenario folder `expected-outputs/IPF-XXX_<path>/` materializes **only the stages whose agents
...
Copilot do **not** have folders under `data-generation/source-exports/ or expected-outputs/` — they run on workflow memory in a full demo;
...
- **Data / systems of record:** sliced from `source-exports/_full_exports/` into each scenario's
