...
The Raw layer is organized **by end-to-end scenario / test case**. **`expected-outputs/IPF-XXX_<path>/` is
...
└── build_scenario_folders.py        ← materializes expected-outputs/IPF-XXX_<path>/ (two MCP stages + scenario.json)
```
...
python3 generate_raw_layer.py          # source-exports/_full_exports/ canonical csv|txt
...
python3 build_scenario_folders.py      # expected-outputs/IPF-XXX_<path>/ (01_signal_ingestion + 02_forecasting + scenario.json)
```
...
exports to `source-exports/_full_exports/`. `generate_normalized_layers.py` and `build_scenario_folders.py`
