...
The canonical exports in source-exports/_full_exports/ plus the normalized layers under
entity-catalog/ and the e2e rollups in ground-truth/07_decision_ground_truth/IPF-XXX.json
are the single sources of truth.

    expected-outputs/IPF-XXX_<path>/
      01_signal_ingestion/
        agent_input.json
...
Idempotent: existing expected-outputs/IPF-*/ folders are removed and rebuilt. Offline (no fetch).
...
GT_DIR = DATA_GEN / "ground-truth" / "07_decision_ground_truth"
CATALOG = DATA_GEN / "entity-catalog"

POS_LAYER = CATALOG / "01_pos_transactions"
SUP_LAYER = CATALOG / "02_supplier_data"
PROMO_LAYER = CATALOG / "03_promotions"
INV_LAYER = CATALOG / "04_inventory"
DMD_LAYER = CATALOG / "05_demand_signals"
POLICY_LAYER = CATALOG / "06_policy_rag"
...
def main() -> None:
    ...
    for scenario in SCENARIOS:
        ...
        scenario_dir = OUTPUT / scenario_folder(scenario)
        files = 0
    ...
    print(f"\nDone — {total_files} files written into per-scenario folders "
          f"under {OUTPUT.relative_to(DATA_GEN)}/ ({len(SCENARIOS)} scenarios)")
